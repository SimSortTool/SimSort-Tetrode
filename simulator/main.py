...
from models.BBP_model import BBPModel
from multiprocessing import Process, RawArray, Value, Lock

def print_structure():
    for sec in neuron.h.allsec():
        print(f"Section: {sec.name()} L={sec.L} diam={sec.diam} nseg={sec.nseg}")

class BBP_population_simulator:
    ...
    def get_model_index(self):
        ...
        self.cell_model_list = glob.glob('BBP_model_train/L*')
    ...
    def setup_simulation(self, idx, probe_pos=None):
        ...
        self.neuron_model = BBPModel(cell_model_folder, start_T=0, end_T=end_T, dt=dt)
...
if __name__ == '__main__':
    ...
    sim = BBP_population_simulator(args.config, args.output_config, script_dir=script_dir, 
